...
const projects = [
  {
    title: 'Modernes Handwerker-Portal',
    category: 'Web Design',
      ...
      <div className="container mx-auto px-5 md:px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-20 gap-6 md:gap-8">
          <div className="reveal-text">
            <span className="text-blue-600 font-bold uppercase tracking-widest text-[10px] md:text-sm mb-2 md:mb-4 block">Portfolio</span>
            <h2 className="text-4xl md:text-6xl font-black text-slate-900 leading-tight">Lokale Erfolge.</h2>
          </div>
          <p className="text-slate-600 max-w-md text-base md:text-lg reveal-text">
            Echte Ergebnisse für echte Unternehmen aus der Region. Entdecken Sie unsere neuesten Web-Lösungen.
          </p>
        </div>
        ...
        <div className="relative group max-w-6xl mx-auto reveal-text">
          <div 
            className="w-full h-[350px] sm:h-[450px] md:h-[600px] rounded-[2rem] md:rounded-[3rem] bg-slate-200 overflow-hidden relative shadow-2xl shadow-slate-200 reveal-container"
          >
            {projects.map((project, index) => (
              ...
              >
                ...
                <div className="absolute bottom-6 left-6 right-6 md:bottom-12 md:left-12 md:right-12 flex flex-col md:flex-row md:items-end justify-between gap-6 z-20">
                  <div className="transform translate-y-0 md:translate-y-4 md:group-hover:translate-y-0 transition-transform duration-500">
                    <span className="text-blue-400 font-bold uppercase tracking-widest text-[10px] mb-1 md:mb-2 block">{project.category}</span>
                    <h3 className="text-2xl md:text-5xl font-black text-white leading-tight">{project.title}</h3>
                  </div>
                  <a 
                    href={project.link} 
                  ...
                  >
                    Projekt ansehen <ExternalLink size={18} className="ml-2" />
                  </a>
                </div>
              </div>
            ))}
            ...
            {projects.map((_, index) => (
              <button 
                ...
                className={`h-2.5 rounded-full transition-all duration-500 interactive ${
                  index === currentIndex ? 'w-8 md:w-16 bg-blue-600 shadow-lg shadow-blue-200' : 'w-2.5 bg-slate-200 hover:bg-slate-400'
                }`}
