...
import { Terminal, Cpu, Layers, ArrowRight, MousePointer2, Smartphone, Search } from 'lucide-react';
...
const Services: React.FC = () => {
  ...
  return (
    <section id="services" className="py-24 md:py-40 bg-slate-950 relative overflow-hidden">
      <div className="container mx-auto px-5 md:px-6">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 md:mb-32 gap-8 md:gap-10">
          <div className="max-w-2xl">
            <span className="text-blue-500 font-black uppercase tracking-[0.3em] text-[10px] md:text-xs mb-4 md:mb-6 block reveal-text">Maßgeschneiderte Pakete</span>
          ...
          <p className="text-slate-400 text-lg md:text-xl font-light max-w-md reveal-text">
            Wir bauen keine komplizierten Systeme, sondern Webseiten, die Ihnen mehr Kunden und weniger Aufwand bringen.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
          {services.map((service, idx) => (
            ...
            >
              ...
              <div className={`w-14 h-14 md:w-20 md:h-20 bg-gradient-to-br ${service.color} rounded-xl md:rounded-2xl flex items-center justify-center text-white mb-6 md:mb-10 shadow-2xl`}>
                <service.icon size={window.innerWidth < 768 ? 28 : 40} />
              </div>
              
              <h3 className="text-2xl md:text-3xl font-black text-white mb-4 md:mb-6 group-hover:text-blue-400 transition-colors">{service.title}</h3>
              ...
              <div className="flex items-center gap-3 text-white font-bold group-hover:gap-5 transition-all text-sm md:text-base">
                Mehr erfahren <ArrowRight size={20} className="text-blue-500" />
              </div>
            </div>
          ))}
        </div>
