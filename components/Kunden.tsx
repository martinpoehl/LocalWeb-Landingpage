      ...
      <div className="container mx-auto px-5 md:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 max-w-7xl mx-auto">
          {kundenDaten.map((stimme, idx) => (
            <div
              key={idx}
              className="reveal-text group relative bg-white p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-2 transition-all duration-500 flex flex-col h-full w-full"
            >
              <div className="absolute -top-3 -right-3 md:-top-5 md:-right-5 w-12 h-12 md:w-16 md:h-16 bg-blue-600/5 text-blue-600/20 rounded-full flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                <Quote size={24} className="md:w-8 md:h-8" />
              </div>

              <div className="flex gap-1 mb-4 md:mb-6">
                {[...Array(stimme.rating)].map((_, i) => (
                  <Star key={i} size={16} className="fill-blue-500 text-blue-500 md:w-4.5 md:h-4.5" />
                ))}
              </div>

              <blockquote className="text-slate-700 text-base md:text-lg leading-relaxed mb-6 md:mb-8 flex-1 font-medium italic">
                "{stimme.content}"
              </blockquote>
              ...
              <div className="flex items-center gap-3 md:gap-4 pt-4 md:pt-6 border-t border-slate-50">
                <img
                  src={stimme.image}
                  alt={stimme.name}
                  className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl object-cover grayscale group-hover:grayscale-0 transition-all duration-500 shadow-md shadow-slate-200"
                />
                <div>
                  <h4 className="text-slate-900 font-black text-sm md:text-lg leading-tight">
                    {stimme.name}
                  </h4>
                  <div className="mt-0.5 md:mt-1">
                    <p className="text-blue-600 text-[9px] md:text-xs font-black uppercase tracking-wider leading-tight">
                      {stimme.role}
                    </p>
                    <p className="text-slate-500 text-[8px] md:text-[11px] font-bold uppercase tracking-widest mt-0.5 md:mt-1">
                      {stimme.company}
                    </p>
                  </div>
